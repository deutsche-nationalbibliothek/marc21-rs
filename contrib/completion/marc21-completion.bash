_marc21() {
    ...
    case "${cmd}" in
        marc21)
            opts="-h -V --help --version concat cat count cnt dedup describe filter frequency freq hash invalid partition print sample select split build-completion build-man"
        ...
        marc21__subcmd__partition)
            opts="-t -o -s -p -c -h --template --output --skip-invalid --strsim-threshold --where --progress --compression --help <PATH> [FILENAMES]..."
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
